...
| Traces and debugging | Source-verified | Source-aware finalized/acquired/imported discovery, compact/detailed frames, typed plan/accepted-attempt lineage, descriptor-first semantic `contentRef` reads and limits, coverage-aware search, retries, usage, failures, optional exact raw forensics, and current-run limitations |
