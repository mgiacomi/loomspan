...
func TestCompositeAdversarialCaseServesUntrustedRuntimeContentThroughMCP(t *testing.T) {
	...
	listed := post(`{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"LOOMSPAN_list_traces","arguments":{"sourceFilter":"IMPORTED"}}}`)
	...
	handle, _ := item["artifactHandle"].(string)
	if handle == "" {
		t.Fatal("adversarial trace did not expose its opaque artifact handle")
	}
	raw := post(fmt.Sprintf(`{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"LOOMSPAN_read_trace_artifact","arguments":{"source":"IMPORTED","artifactHandle":%q,"start":0}}}`, handle))
